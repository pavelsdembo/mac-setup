...
auto-updates. The Homebrew cask does not.
...
**Menu bar**  AirDrop hidden, Control Centre shown, analog clock with day of
week and AM/PM
...
**Stats**  Battery widget with a mini readout, clock on, disk off
...
**VS Code**  Settings and extensions from `home/vscode/`
...
2. **Privacy & Security > Automation** - allow your terminal to control System
   Events, or login items are skipped with a warning.
3. **Raycast** - sign in and enable Cloud Sync. Hotkeys, aliases, snippets
   and extensions are stored server-side.
4. **Hidden Bar** - Cmd-drag menu bar icons left of its divider.
5. **VS Code** - recreate any profiles.
6. **Git identity** - not managed here:
   ```sh
